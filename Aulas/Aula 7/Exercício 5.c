...
int busca_binaria(int v[], int n, int e) {
    ...
    // VARIÁVEL ADICIONADA: Contador de iterações do laço
    int contador = 0; 

    while (maior_indice >= menor_indice) {
        // INCREMENTO DO CONTADOR: A cada execução do laço
        contador++; 
        
        indice_meio = (maior_indice + menor_indice) / 2;

        if (v[indice_meio] == e) {
            // IMPRIME A QUANTIDADE DE VEZES ANTES DE RETORNAR
            printf("--- A busca binária levou %d iterações para encontrar o elemento %d.\n", contador, e);
            return indice_meio;
        }
    ...
    // IMPRIME A QUANTIDADE DE VEZES ANTES DE RETORNAR -1
    printf("--- A busca binária levou %d iterações, mas o elemento %d não foi encontrado.\n", contador, e);
    return -1;
}
